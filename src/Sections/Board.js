...
export default function Board({data, setData}){
    ...
    return(
        ...
        <Gallery>
        {data.map((item, index) => (
          <Cards key={index} gradient={rightshade} direction={'to bottom'} direction2={'to bottom'}  min={'100%'} max={'fit-content'} height={'fit-content'} resize={'100%'} hr={'fit-content'} 
          style={{ boxShadow:'0px 0px 10px rgba(0,0,0,0.5)', backgroundColor:isColor(`${item.Cardcolor}`)? `${item.Cardcolor}`:'black', color: isColor(`${item.Fontcolor}`)? `${item.Fontcolor}`:'pink' }}>
    ...
    display: 'flex',alignItems: 'center', flexWrap: 'wrap', marginTop:'-8%'}}>
      <ThumbUpAltIcon onClick={()=> handleLike(item.id,item.Likes, index)} sx={{ fontSize: 25, m:'3%', "&:hover": { color: "LightGreen" }}}/>
     <p>{item.Likes}</p>
      </div>
        </Cards>
        ))}
